...
import s from './scss/Accordion.module.scss';
...
const Accordion: FC<AccordionProps> = function({summary, onClick, open: propOpen, onToggle, closeOnMouseOut, keepOpen, openDirection = 'down', children, closeOnClick}) {
	...
	const child: (t: JSX.Element) => JSX.Element = (t) => {
		const props = {
			className: `${s['details-container']}${open ? ` ${s.open}` : ''}`,
			onClick: () => {
				if (!closeOnClick){
			...
			t?.type === 'tr'
				...
				>
					<div {...(t.type === 'div' ? {...t.props,
						...
						className: `${s.details}${t.props.className ? ` ${t.props.className}` : ''}`
					} : {className: `${s.details}${t.props.className ? ` ${t.props.className}` : ''}`})} >{t.type === 'div' ? t.props.children : t}</div>
				</div>
			: <div {...props} style={{height: dimensions.y, width: dimensions.x}}>
				<div ref={details as LegacyRef<HTMLDivElement>} className={s.details}>{t}</div>
			</div>
	}
...
const AccordionRow: FC<AccordionRowProps> = ({details, children, ...props}) => {
